...
# Final Collumns
COLLUMNS = ['cod_ibge', 'id_mun', 'ano', 'casos']
...
def preprocess_dengue_data(df):
    ...
    df = pd.melt(df, id_vars='id_mun', var_name='year', value_name='cases')
...
def merge_dengue_data(*dfs):
    ...
    df_merged['total'] = df_merged.filter(like='cases').sum(axis=1).astype('int64')
    df_merged.drop(columns=df_merged.filter(like='cases').columns, inplace=True)
    df_merged.rename(columns={'year': 'ano', 'total': 'casos'}, inplace=True)
...
def main():
    ...
    df_1324 = load_csv(RAW_DIR + 'casos_dengue_MU_BR_2013_2024.csv', encoding=ENCODING, skiprows=SKIPROWS, nrows=774, sep=SEP)
    df_0713 = load_csv(RAW_DIR + 'casos_dengue_MU_BR_2007_2013.csv', encoding=ENCODING, skiprows=SKIPROWS, nrows=775, sep=SEP)
    df_0106 = load_csv(RAW_DIR + 'casos_dengue_MU_BR_2001_2006.csv', encoding=ENCODING, skiprows=SKIPROWS, nrows=741, sep=SEP)
